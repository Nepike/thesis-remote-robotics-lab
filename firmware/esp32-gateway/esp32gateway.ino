...
//
// ВНИМАНИЕ: здесь НЕ должно быть client.flush(). В Arduino-core для ESP32 версии
// 2.x WiFiClient::flush() не «проталкивает» отправку, а наоборот — вычитывает и
// ВЫБРАСЫВАЕТ всё, что уже пришло в приёмный буфер. Этот вызов стоял здесь и
// молча съедал команды сервера, если они приходили в момент, когда буфер UART
// заполнился (вызов из loop() по достижении TCP_BUF_SIZE — там, в отличие от
// второго места вызова, client.available() не проверяется). Наружу это выглядело
// как потери в канале rosserial.
//
// Проталкивать ничего и не нужно: setNoDelay(true) при подключении клиента
// отключает алгоритм Нейгла, поэтому write() уходит в сеть немедленно.
void flushTcpBuffer() {
  if (tcpBufPos == 0) return;

  if (client.connected()) {
    client.write(tcpBuffer, tcpBufPos);   // отправляем накопленные байты пачкой
  }
  // Позицию сбрасываем в любом случае: если клиента уже нет, эти байты всё равно
  // некому отдать, а оставленный непустым буфер — это заявка на переполнение
  // tcpBuffer при следующей записи.
  tcpBufPos = 0;
}
